...
function createEmptyPayload(): PopupState {
  return {
...
function dispatch(command: PopupCommand): Promise<PopupState | DesktopRequestResult> {
  return sendRuntimeMessage<PopupState | DesktopRequestResult>(command);
}
...
export function usePopupBridge(activeView: PopupView) {
  const [payload, setPayload] = useState<PopupState>(createEmptyPayload);
