...
import {buildDownloadSpec, filenameForDesktop, resourceNameFromCapture} from "./download-spec";
...
export function createResourceBridge(options: {
  ...
  async function tabIdForNetworkRequest(details: chrome.webRequest.OnResponseStartedDetails): Promise<number | null> {
    ...
    const snapshotTabId = headerSnapshotByUrl(details.url)?.tabId;
  ...
  function headerSnapshotByUrl(url: string): HeaderSnapshot | null {
    pruneHeaderSnapshots();
  ...
  function cacheUrlResource(
    ...
    const headerSnapshot = headerSnapshotByUrl(url);
    const headers = {
  ...
  async function handoffBrowserDownload(downloadItem: chrome.downloads.DownloadItem) {
    ...
    const headerSnapshot = headerSnapshotByUrl(finalUrl) ?? headerSnapshotByUrl(downloadItem.url);
  ...
  async function resourceForMediaUrl(url: string, tabId: number, fallbackTitle: string, fallbackPageUrl: string): Promise<CapturedResource> {
    ...
    const snapshot = headerSnapshotByUrl(url);
  ...
  async function dispatchExternalDownload(
    ...
    return options.sendDesktopRequest<DesktopRequestResult>({
      ...
      payload: {
        ...
        pageTitle: title || "",
        headers: headersForPage(selection.pageUrl),
      },
    });
  }
  ...
  function headersForPage(pageUrl: string): Record<string, string> {
    const host = hostOf(pageUrl);
    if (host === null) {
      return {};
    }

    let snapshot = headerSnapshotByUrl(pageUrl);
